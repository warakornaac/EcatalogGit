...
function clearAllFilters() {
  ...
  vfData = {};
  ['vf-market','vf-segment','vf-maker','vf-range','vf-body','vf-engine','vf-drive'].forEach(id => {
    const el = gEl(id); if (el) el.value = '';
  });
  const yrFrom = gEl('vf-yr-from'); if (yrFrom) yrFrom.value = '';
  const yrTo   = gEl('vf-yr-to');   if (yrTo)   yrTo.value   = '';
...
function buildSpecHTML(p) {
  return `
    <div class="spec-hero">
      ...
      <div class="spec-hero-meta flex-grow-1">
        ...
        <div class="mt-2 d-flex align-items-center gap-3">
          <div class="spec-price">$${p.price.toFixed(2)}</div>
    ...
    <div class="stabs-nav">
      ...
      <button class="stab-btn" onclick="switchTabIn(this,'spec','${p.id}')"><i class="bi bi-rulers"></i> Spec <span class="tb">8</span></button>
      <button class="stab-btn" onclick="switchTabIn(this,'img','${p.id}')"><i class="bi bi-images"></i> Image</button>
      <button class="stab-btn" onclick="switchTabIn(this,'oem','${p.id}')"><i class="bi bi-upc"></i> OEM <span class="tb">4</span></button>
      <button class="stab-btn" onclick="switchTabIn(this,'comp','${p.id}')"><i class="bi bi-diagram-2"></i> Competitor</button>
      <button class="stab-btn" onclick="switchTabIn(this,'veh','${p.id}')"><i class="bi bi-car-front"></i> Vehicle <span class="tb">6</span></button>
    </div>
    <div class="stab-content">
      <div class="stab-pane active" id="itab-desc-${p.id}">
        <p style="font-size:13px;line-height:1.7;color:var(--text-2)">
          Premium quality ${p.name.toLowerCase()} engineered to OEM specifications for optimal performance and reliability. Manufactured with advanced materials ensuring extended service life and perfect fitment.
        </p>
        <div class="row g-2 mt-1">
          ...
          <div class="col-6 col-sm-3"><div style="background:var(--surface-2);border-radius:7px;padding:9px;text-align:center;border:1px solid var(--border)"><div style="font-size:10px;color:var(--text-3);text-transform:uppercase;font-weight:600">Warranty</div><div style="font-size:12px;font-weight:600;margin-top:3px">24 Months</div></div></div>
          <div class="col-6 col-sm-3"><div style="background:var(--surface-2);border-radius:7px;padding:9px;text-align:center;border:1px solid var(--border)"><div style="font-size:10px;color:var(--text-3);text-transform:uppercase;font-weight:600">Origin</div><div style="font-size:12px;font-weight:600;margin-top:3px">Japan</div></div></div>
      ...
      <div class="stab-pane" id="itab-spec-${p.id}">
        <table class="spec-table">
          <tr><td>Part Number</td><td>${p.code}</td></tr>
          ...
          <tr><td>Dimensions</td><td>148 × 64 × 18 mm</td></tr>
          <tr><td>Weight</td><td>0.85 kg / set</td></tr>
          <tr><td>Material</td><td>Ceramic Composite</td></tr>
          <tr><td>Friction Coefficient</td><td>µ = 0.38–0.45</td></tr>
          <tr><td>Max Temperature</td><td>450 °C</td></tr>
          <tr><td>Noise Level</td><td>&lt; 70 dB</td></tr>
          <tr><td>Certification</td><td>ECE R90, ISO 6312</td></tr>
        </table>
      </div>
      <div class="stab-pane" id="itab-img-${p.id}">
        <div class="img-grid">
          ...
          <div class="img-ph"><i class="bi bi-diagram-3"></i><span>Exploded Diagram</span></div>
          <div class="img-ph"><i class="bi bi-rulers"></i><span>Dimension Drawing</span></div>
        </div>
      </div>
      <div class="stab-pane" id="itab-oem-${p.id}">
        <p style="font-size:12px;color:var(--text-3);margin-bottom:10px">OEM cross-reference numbers:</p>
        <div class="oem-chips">
          <span class="oem-chip oem-a"><i class="bi bi-upc-scan"></i> 04465-06260</span>
          <span class="oem-chip oem-a"><i class="bi bi-upc-scan"></i> 04465-0E010</span>
          <span class="oem-chip oem-b"><i class="bi bi-upc-scan"></i> 45022-T2A-A01</span>
          <span class="oem-chip oem-c"><i class="bi bi-upc-scan"></i> 3421-1160-469</span>
        </div>
      </div>
      <div class="stab-pane" id="itab-comp-${p.id}">
        <table class="spec-table">
          <thead><tr style="border-bottom:1.5px solid var(--border)"><td style="font-size:11px;font-weight:700;text-transform:uppercase;color:var(--text)">Brand</td><td style="font-size:11px;font-weight:700;text-transform:uppercase;color:var(--text)">Part No.</td></tr></thead>
          <tr><td>Brembo</td><td>P28 013</td></tr>
          <tr><td>Bosch</td><td>BP747</td></tr>
          <tr><td>ATE</td><td>13.0460-2741.2</td></tr>
          <tr><td>Ferodo</td><td>FDB1172</td></tr>
          <tr><td>Mintex</td><td>MDB2041</td></tr>
        </table>
      </div>
      <div class="stab-pane" id="itab-veh-${p.id}">
        <div class="veh-list">
          <div class="veh-item"><i class="bi bi-car-front-fill"></i><div><strong>Toyota Camry</strong> XV70 2.5L 2018–2024 (FWD/AWD)</div></div>
          <div class="veh-item"><i class="bi bi-car-front-fill"></i><div><strong>Toyota Avalon</strong> XX50 3.5L 2019–2022 (FWD)</div></div>
          <div class="veh-item"><i class="bi bi-car-front-fill"></i><div><strong>Lexus ES350</strong> XZ10 3.5L 2019–2024 (FWD)</div></div>
          <div class="veh-item"><i class="bi bi-car-front-fill"></i><div><strong>Toyota RAV4</strong> XA50 2.5L 2019–2024 (FWD/AWD)</div></div>
          <div class="veh-item"><i class="bi bi-car-front-fill"></i><div><strong>Lexus NX300h</strong> AZ20 2.0L 2018–2022 (Hybrid)</div></div>
          <div class="veh-item"><i class="bi bi-car-front-fill"></i><div><strong>Toyota Venza</strong> AV80 2.5L 2021–2024 (FWD)</div></div>
        </div>
      </div>
    </div>`;
}
...
function openDrawer(id, e) {
  ...
  if (isMobile()) {
    ...
    gEl('drPrice').textContent    = '$' + p.price.toFixed(2);
    gEl('drDescFull').textContent = `Premium quality ${p.name.toLowerCase()} engineered to OEM specifications for optimal performance and reliability. Manufactured with advanced materials ensuring extended service life and perfect fitment.`;
...
function renderProducts(list) {
  ...
  if (!sorted.length) {
    const noHTML = '<div class="no-results"><i class="bi bi-search"></i><p>No products match.<br>Try adjusting the filters.</p></div>';
    if (isGrouped) {
...
function removeVfTag(k) {
  delete vfData[k];
  const selId = `vf-${k}`;
  const sel = gEl(selId); if (sel) sel.value = '';
  renderVfTags();
  updateVehSummary();
  applyAllFilters();
}
...
function buildSuggestions(q) {
  ...
  PRODUCTS.forEach(p => {
    ...
    if (nameLow.includes(qLow) || codeLow.includes(qLow) || brandLow.includes(qLow) || catLow.includes(qLow)) {
      ...
      if (!seen.has(key)) {
        seen.add(key);
        results.push({ type:'product', id:p.id, name:p.name, code:p.code, brand:p.brand||'', cat:p.category||'', price:p.price });
      }
    }
  });
...
//             const allowed = currentAllowed[type] || [];
...
//     $("#plList .chk-item").each(function () {
//         const id = $(this).attr('data-id');
//         const show = allowedIds.length === 0 || allowedIds.includes(id);
//         $(this).toggle(show);
//     });
// }
function FilterProductionLines(allowedIds) {
